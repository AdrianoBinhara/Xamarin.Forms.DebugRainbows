...
public class DebugGridViewDroid : View
{
    ...
    protected override void OnDraw(Canvas canvas)
    {
        ...
        else
        {
            if (MakeGridRainbows)
            {
                ...
                var shader = new LinearGradient(canvas.Width - (float)a, (float)b, canvas.Width - (float)c, (float)d, (int[])colors.Select(x => (int)x.ToArgb()).ToArray(), locations, Shader.TileMode.Clamp);
    ...
    private void DrawInverse(Canvas canvas, Paint majorPaint, Android.Graphics.Color[] colors)
    {
